...
class McaoSearchList extends HookWidget {
  McaoSearchList({required this.navClose});
  final bool navClose;
  @override
  Widget build(BuildContext context) {
    
    ValueNotifier searchString =  useState(context.read<Daily10Provider>().daily10Search);
    final List<McaoSearchModel>? dailyDetails =
        context.select((McaoProvider p) => p.mcaoProvList);
  ...
  Widget searchList(BuildContext context, McaoSearchModel? dailyMods) {
        ...
        ? Column(children: [
            ...
            GestureDetector(
              onTap: (){
                  ...
                  dailyProvider.setProvID(dailyMods.id);  
